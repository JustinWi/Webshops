...
var firebaseRoot = "https://cdlwebshops.firebaseio.com/swatlantic";
...
app.controller('MainCtrl', ['$scope', '$firebaseArray', '$firebaseAuth', '$firebaseObject', 'uiGridConstants', function ($scope, $firebaseArray, $firebaseAuth, $firebaseObject, uiGridConstants) {
  ...
  vm2.earlyAdoptersFeedbackGrid = {
    columnDefs: [
      ...
      },
      {
        name: 'select',
        displayName: 'Select',
        cellTemplate: '<button id="selectBtn" type="button" class="btn-small" ng-click="grid.appScope.vm.giveFeedbackEarlyAdopters(row.entity)" >Select</button> ',
        width: 60,
        enableSorting: false
      }
    ],
  ...
  if (isSimpleMode()) {
    ...
    vm2.giveFeedbackEarlyAdopters = function (exercise) {
