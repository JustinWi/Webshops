...
const DISPLAY_VIDEO = false;

var firebaseRoot = "https://cdlwebshops.firebaseio.com/swtoronto";
...
function initScriptForm() {
  ...
  console.log("running init forms");
  ...
  $('#scriptForm').bootstrapValidator({
    ...
    if (event.isDefaultPrevented()) {
      ...
      var interviewType = $("input[type='radio'][name='interviewType']:checked");
      var scriptType = "";
      if (interviewType.length > 0) {
        scriptType = interviewType.attr('id');
      }

      var urlStr = "https://script.google.com/macros/s/AKfycbyBwZkAPscgFmd6jDDRVPUiL02YgT0-vzsA4JTDFndD5ibXClU/exec?email=" +
        encodeURIComponent($('#scriptEmail').val()) +
        "&role=" + encodeURIComponent($('#customerRole').val()) +
        "&context=" + encodeURIComponent($('#problemContext').val()) +
        "&problem=" + encodeURIComponent($('#problem').val()) +
        "&script=" + scriptType +
        "&sendEmail=true";

      console.log('Invoking: ' + urlStr);
...
function isWebViewerMode() {
  return DISPLAY_VIDEO && !isSimpleMode() && document.URL.indexOf("vip.html") == -1;
}
